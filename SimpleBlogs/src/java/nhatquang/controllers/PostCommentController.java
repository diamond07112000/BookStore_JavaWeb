...
public class PostCommentController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            while (true) {
                CommentDTO dto = commentDAO.getCommentByCommentId(commentId);
                if (dto != null) {
                    commentId += 1;
                } else {
                    break;
            ...
            CommentDTO commentDTO = new CommentDTO(commentId, "", comment, Integer.parseInt(blogId), dateOfCreate, false);
            boolean check = commentDAO.insertComment(commentDTO, user.getEmail());
            if(check){
                request.setAttribute("MESS", "Post success!");
                BlogDAO blogDAO = new BlogDAO();
                BlogDTO blogDTO = blogDAO.getBlogByBlogId(Integer.parseInt(blogId));
                List<CommentDTO> listComment = commentDAO.getAllCommentByBlogId(Integer.parseInt(blogId));
                blogDTO.setListComment(listComment);
                session.setAttribute("BLOG", blogDTO);
            } else {
